...
export class FlexibleTableMobileVScrollComponent extends FlexibleTableMobileComponent implements AfterViewInit {
  ...
  getItemSize() {
    var result = 0;
    if (this.getTopColumns().length) {
      result += this.topHeight;
    }
    if (this.getMiddleColumns().length) {
      result += this.middleHeight;
    }
    if (this.getBottomColumns().length) {
      result += this.bottomHeight;
    }
    return result;
  }
}